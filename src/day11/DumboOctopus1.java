...
public class DumboOctopus1 {
    ...
    public static void main(String[] args) {
        ...
        for (int step = 1; step <= STEPS; step++) {
            ...
            numberOfFlashes += getNumberOfFlashes(input);
            resetFlashes(input);
        }
    ...
    private static void incrementEnergyLevels(int[][] energyLevels, int i, int j) {
        if (++energyLevels[i][j] == FLASH) {
            if (areOnMatrix(i - 1) && energyLevels[i - 1][j] < FLASH) {
                incrementEnergyLevels(energyLevels, i - 1, j);
            }
            if (areOnMatrix(i + 1) && energyLevels[i + 1][j] < FLASH) {
                incrementEnergyLevels(energyLevels, i + 1, j);
            }
            if (areOnMatrix(j - 1) && energyLevels[i][j - 1] < FLASH) {
                incrementEnergyLevels(energyLevels, i, j - 1);
            }
            if (areOnMatrix(j + 1) && energyLevels[i][j + 1] < FLASH) {
                incrementEnergyLevels(energyLevels, i, j + 1);
            }
            if (areOnMatrix(i - 1, j - 1) && energyLevels[i - 1][j - 1] < FLASH) {
    ...
    private static boolean areOnMatrix(int... indices) {
